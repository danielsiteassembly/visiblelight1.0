(function ($) {
  ...
  function renderPrettyReport(r) {
    ...
    var score = (summary && typeof summary.score !== "undefined") ? summary.score : null;
    var errorDensity = summary.error_density !== undefined ? summary.error_density : null;
    var uniqueIssues = summary.unique_issues !== undefined ? summary.unique_issues : null;
    var wcagCompliance = summary.wcag_compliance || {};
    var userImpact = summary.user_impact || {};
    var keyboardScore = summary.keyboard_accessibility_score !== undefined ? summary.keyboard_accessibility_score : null;
    var screenReaderScore = summary.screen_reader_compatibility !== undefined ? summary.screen_reader_compatibility : null;
    
    var headHtml =
      '<div class="vl-las-report-head" style="padding:8px 10px;border:1px solid #ccd0d4;border-radius:6px;background:#fff;margin-bottom:12px;">' +
      '<div><strong>URL:</strong> ' + escapeHtml(r.url || "(n/a)") + "</div>" +
      '<div><strong>When:</strong> ' + escapeHtml(r.created_at ? asDate(r.created_at) : "(n/a)") + "</div>" +
      '<div><strong>Passed:</strong> ' + escapeHtml(summary.passed ?? "-") + " / " + escapeHtml(summary.total ?? "-") + "</div>" +
      '<div><strong>Score:</strong> ' + (score !== null ? escapeHtml(score + "%") : "—") + "</div>" +
      "</div>";
    $wrap.append($(headHtml));
    
    // New Metrics Section
    var metricsHtml = '<div class="vl-las-report-metrics" style="padding:12px;border:1px solid #ccd0d4;border-radius:6px;background:#f9f9f9;margin-bottom:12px;">' +
      '<h3 style="margin-top:0;margin-bottom:12px;font-size:16px;">Accessibility Metrics</h3>' +
    ...
    if (errorDensity !== null) {
      ...
      metricsHtml += '<div style="padding:8px;background:#fff;border-radius:4px;border:1px solid #ddd;">' +
        '<div style="font-size:24px;font-weight:bold;color:#d63638;">' + escapeHtml(errorDensity.toFixed(1)) + '%</div>' +
        '<div style="font-size:12px;color:#666;margin-top:4px;">Error Density</div>' +
        '<details style="margin-top:8px;font-size:11px;"><summary style="cursor:pointer;color:#2271b1;">Show calculation</summary>' +
        '<div style="margin-top:6px;padding:6px;background:#f9f9f9;border-radius:3px;font-size:10px;">' +
        '<div><strong>Failed Checks:</strong> ' + escapeHtml(failed) + '</div>' +
        '<div><strong>Total Checks:</strong> ' + escapeHtml(summary.total || 0) + '</div>';
      if (failedChecks.length > 0) {
        metricsHtml += '<div style="margin-top:6px;"><strong>Failed Check Types:</strong><ul style="margin:4px 0 0 20px;padding:0;">';
        failedChecks.forEach(function(name) {
          metricsHtml += '<li>' + escapeHtml(name) + '</li>';
        });
        metricsHtml += '</ul></div>';
      }
      metricsHtml += '<div style="margin-top:4px;color:#666;">Score: (' + escapeHtml(failed) + ' / ' + escapeHtml(summary.total || 0) + ') × 100 = ' + escapeHtml(errorDensity.toFixed(1)) + '%</div>' +
        '</div></details>' +
        '</div>';
    }
    ...
    if (uniqueIssues !== null) {
      ...
      metricsHtml += '<div style="padding:8px;background:#fff;border-radius:4px;border:1px solid #ddd;">' +
        '<div style="font-size:24px;font-weight:bold;color:#d63638;">' + escapeHtml(uniqueIssues) + '</div>' +
        '<div style="font-size:12px;color:#666;margin-top:4px;">Unique Issues</div>' +
        '<details style="margin-top:8px;font-size:11px;"><summary style="cursor:pointer;color:#2271b1;">Show calculation</summary>' +
        '<div style="margin-top:6px;padding:6px;background:#f9f9f9;border-radius:3px;font-size:10px;">' +
        '<div><strong>Failure Type Definition:</strong> Each unique accessibility check that failed, regardless of how many instances exist on the page.</div>';
      if (uniqueFailedChecks.length > 0) {
        metricsHtml += '<div style="margin-top:6px;"><strong>Unique Failure Types Found:</strong><ul style="margin:4px 0 0 20px;padding:0;">';
        uniqueFailedChecks.forEach(function(name) {
          metricsHtml += '<li>' + escapeHtml(name) + '</li>';
        });
        metricsHtml += '</ul></div>';
      }
      metricsHtml += '<div style="margin-top:4px;color:#666;">Total unique failure types: ' + escapeHtml(uniqueIssues) + '</div>' +
        '</div></details>' +
        '</div>';
    }
    
    // WCAG Compliance
    if (wcagCompliance.A || wcagCompliance.AA || wcagCompliance.AAA) {
      var wcagHtml = '';
      if (wcagCompliance.A) {
        var aScore = wcagCompliance.A.score || 0;
        var aCompliant = wcagCompliance.A.compliant ? '✓' : '✗';
        wcagHtml += '<div style="margin-top:4px;"><strong>Level A:</strong> ' + escapeHtml(aScore) + '% ' + aCompliant + ' (' + escapeHtml(wcagCompliance.A.passed || 0) + '/' + escapeHtml(wcagCompliance.A.total || 0) + ' passed)</div>';
      }
      if (wcagCompliance.AA) {
        var aaScore = wcagCompliance.AA.score || 0;
        var aaCompliant = wcagCompliance.AA.compliant ? '✓' : '✗';
        wcagHtml += '<div style="margin-top:4px;"><strong>Level AA:</strong> ' + escapeHtml(aaScore) + '% ' + aaCompliant + ' (' + escapeHtml(wcagCompliance.AA.passed || 0) + '/' + escapeHtml(wcagCompliance.AA.total || 0) + ' passed)</div>';
      }
      if (wcagCompliance.AAA) {
        var aaaScore = wcagCompliance.AAA.score || 0;
        var aaaCompliant = wcagCompliance.AAA.compliant ? '✓' : '✗';
        wcagHtml += '<div style="margin-top:4px;"><strong>Level AAA:</strong> ' + escapeHtml(aaaScore) + '% ' + aaaCompliant + ' (' + escapeHtml(wcagCompliance.AAA.passed || 0) + '/' + escapeHtml(wcagCompliance.AAA.total || 0) + ' passed)</div>';
      }
      var wcagDetails = '';
      var checkMapping = {
      ...
      metricsHtml += '<div style="padding:8px;background:#fff;border-radius:4px;border:1px solid #ddd;grid-column:span 2;">' +
        '<div style="font-size:14px;font-weight:bold;margin-bottom:4px;">WCAG Compliance</div>' +
        wcagHtml +
        '<details style="margin-top:8px;font-size:11px;"><summary style="cursor:pointer;color:#2271b1;">Show calculation</summary>' +
        '<div style="margin-top:6px;padding:6px;background:#f9f9f9;border-radius:3px;font-size:10px;">' +
        '<div><strong>WCAG 2.1 Standards:</strong></div>' +
        '<div style="margin-top:4px;"><strong>Level A:</strong> Basic accessibility requirements. Must be met for minimum accessibility compliance.</div>' +
        '<div style="margin-top:4px;"><strong>Level AA:</strong> Enhanced accessibility requirements. Recommended for most websites and required for many organizations.</div>' +
        '<div style="margin-top:4px;"><strong>Level AAA:</strong> Highest level of accessibility. Optional and often difficult to achieve for all content.</div>' +
        '<div style="margin-top:6px;color:#666;">Checks are mapped to WCAG 2.1 Levels based on their criteria. Score = (Passed Checks / Total Checks) × 100 for each level. Compliant = All checks for that level passed.</div>' +
        wcagDetails +
        '</div></details>' +
        '</div>';
    }
    ...
    // User Impact
    if (userImpact.critical !== undefined || userImpact.high !== undefined || userImpact.medium !== undefined || userImpact.low !== undefined) {
      var impactHtml = '';
      if (userImpact.critical > 0) impactHtml += '<div style="margin-top:4px;"><span style="color:#d63638;">●</span> <strong>Critical:</strong> ' + escapeHtml(userImpact.critical) + '</div>';
      if (userImpact.high > 0) impactHtml += '<div style="margin-top:4px;"><span style="color:#f56e28;">●</span> <strong>High:</strong> ' + escapeHtml(userImpact.high) + '</div>';
      if (userImpact.medium > 0) impactHtml += '<div style="margin-top:4px;"><span style="color:#f0b849;">●</span> <strong>Medium:</strong> ' + escapeHtml(userImpact.medium) + '</div>';
      if (userImpact.low > 0) impactHtml += '<div style="margin-top:4px;"><span style="color:#00a32a;">●</span> <strong>Low:</strong> ' + escapeHtml(userImpact.low) + '</div>';
      if (impactHtml) {
        metricsHtml += '<div style="padding:8px;background:#fff;border-radius:4px;border:1px solid #ddd;grid-column:span 2;">' +
          '<div style="font-size:14px;font-weight:bold;margin-bottom:4px;">User Impact</div>' +
          impactHtml +
          '</div>';
      }
    }
    ...
    if (keyboardScore !== null) {
      var kbDetails = summary.keyboard_accessibility_details;
      var kbColor = keyboardScore >= 80 ? '#00a32a' : keyboardScore >= 60 ? '#f0b849' : '#d63638';
      metricsHtml += '<div style="padding:8px;background:#fff;border-radius:4px;border:1px solid #ddd;">' +
        '<div style="font-size:24px;font-weight:bold;color:' + kbColor + ';">' + escapeHtml(keyboardScore) + '%</div>' +
        '<div style="font-size:12px;color:#666;margin-top:4px;">Keyboard Accessibility</div>';
      if (kbDetails && kbDetails.breakdown) {
        var kbBreakdown = kbDetails.breakdown;
        metricsHtml += '<details style="margin-top:8px;font-size:11px;"><summary style="cursor:pointer;color:#2271b1;">Show calculation</summary>' +
          '<div style="margin-top:6px;padding:6px;background:#f9f9f9;border-radius:3px;">' +
          '<div><strong>Total Interactive Elements:</strong> ' + escapeHtml(kbDetails.total_elements || 0) + '</div>' +
          '<div><strong>Accessible Elements:</strong> ' + escapeHtml(kbDetails.accessible_elements || 0) + '</div>' +
          '<div style="margin-top:6px;font-size:10px;">' +
          '<div>• Buttons: ' + escapeHtml(kbBreakdown.buttons.total || 0) + ' total, ' + escapeHtml(kbBreakdown.buttons.accessible || 0) + ' accessible ' + (kbBreakdown.buttons.check_passed ? '✓' : '✗') + '</div>' +
          '<div>• Links: ' + escapeHtml(kbBreakdown.links.total || 0) + ' total, ' + escapeHtml(kbBreakdown.links.accessible || 0) + ' accessible ' + (kbBreakdown.links.check_passed ? '✓' : '✗') + '</div>' +
          '<div>• Forms: ' + escapeHtml(kbBreakdown.forms.total || 0) + ' total, ' + escapeHtml(kbBreakdown.forms.accessible || 0) + ' accessible ✓</div>' +
          '</div>' +
          '<div style="margin-top:4px;font-size:10px;color:#666;">Score: (' + escapeHtml(kbDetails.accessible_elements || 0) + ' / ' + escapeHtml(kbDetails.total_elements || 0) + ') × 100 = ' + escapeHtml(keyboardScore) + '%</div>' +
          '</div></details>';
      }
      metricsHtml += '</div>';
    }
    ...
    if (screenReaderScore !== null) {
      var srDetails = summary.screen_reader_compatibility_details;
      var srColor = screenReaderScore >= 80 ? '#00a32a' : screenReaderScore >= 60 ? '#f0b849' : '#d63638';
      metricsHtml += '<div style="padding:8px;background:#fff;border-radius:4px;border:1px solid #ddd;">' +
        '<div style="font-size:24px;font-weight:bold;color:' + srColor + ';">' + escapeHtml(screenReaderScore) + '%</div>' +
        '<div style="font-size:12px;color:#666;margin-top:4px;">Screen Reader Compatibility</div>';
      if (srDetails && srDetails.breakdown) {
        var srBreakdown = srDetails.breakdown;
        metricsHtml += '<details style="margin-top:8px;font-size:11px;"><summary style="cursor:pointer;color:#2271b1;">Show calculation</summary>' +
          '<div style="margin-top:6px;padding:6px;background:#f9f9f9;border-radius:3px;">' +
          '<div><strong>Total Elements:</strong> ' + escapeHtml(srDetails.total_elements || 0) + '</div>' +
          '<div><strong>Compatible Elements:</strong> ' + escapeHtml(srDetails.compatible_elements || 0) + '</div>' +
          '<div style="margin-top:6px;font-size:10px;">' +
          '<div>• Images: ' + escapeHtml(srBreakdown.images.total || 0) + ' total, ' + escapeHtml(srBreakdown.images.compatible || 0) + ' compatible ' + (srBreakdown.images.check_passed ? '✓' : '✗') + '</div>' +
          '<div>• Buttons: ' + escapeHtml(srBreakdown.buttons.total || 0) + ' total, ' + escapeHtml(srBreakdown.buttons.compatible || 0) + ' compatible ' + (srBreakdown.buttons.check_passed ? '✓' : '✗') + '</div>' +
          '<div>• Links: ' + escapeHtml(srBreakdown.links.total || 0) + ' total, ' + escapeHtml(srBreakdown.links.compatible || 0) + ' compatible ' + (srBreakdown.links.check_passed ? '✓' : '✗') + '</div>' +
          '<div>• Headings: ' + escapeHtml(srBreakdown.headings.total || 0) + ' total, ' + escapeHtml(srBreakdown.headings.compatible || 0) + ' compatible ' + (srBreakdown.headings.check_passed ? '✓' : '✗') + '</div>' +
          '<div>• ARIA: ' + escapeHtml(srBreakdown.aria.total || 0) + ' total, ' + escapeHtml(srBreakdown.aria.compatible || 0) + ' compatible ' + (srBreakdown.aria.check_passed ? '✓' : '✗') + '</div>' +
          '<div>• Document: Title ' + (srBreakdown.document.title ? '✓' : '✗') + ', Lang ' + (srBreakdown.document.lang ? '✓' : '✗') + '</div>' +
          '</div>' +
          '<div style="margin-top:4px;font-size:10px;color:#666;">Score: (' + escapeHtml(srDetails.compatible_elements || 0) + ' / ' + escapeHtml(srDetails.total_elements || 0) + ') × 100 = ' + escapeHtml(screenReaderScore) + '%</div>' +
          '</div></details>';
      }
      metricsHtml += '</div>';
    }
